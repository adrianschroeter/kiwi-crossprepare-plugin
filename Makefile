...
version := $(shell \
	$(python) -c \
	'from kiwi_crossprepare_plugin.version import __version__; print(__version__)'\
)
...
install:
	...
	gzip -f doc/build/man/kiwi::system::crossprepare.8
	install -m 644 doc/build/man/kiwi::system::crossprepare.8.gz \
		${buildroot}usr/share/man/man8
	install -d -m 755 ${buildroot}${docdir}/python-kiwi_crossprepare_plugin
	install -m 644 LICENSE \
		${buildroot}${docdir}/python-kiwi_crossprepare_plugin/LICENSE
	install -m 644 README.rst \
		${buildroot}${docdir}/python-kiwi_crossprepare_plugin/README
...
build: clean tox
	...
	> dist/python-kiwi_crossprepare_plugin.changes
	helper/update_changelog.py \
		--file package/python-kiwi_crossprepare_plugin.changes \
	>> dist/python-kiwi_crossprepare_plugin.changes
	# update package version in spec file
	cat package/python-kiwi_crossprepare_plugin-spec-template |\
		sed -e s'@%%VERSION@${version}@' \
	> dist/python-kiwi_crossprepare_plugin.spec
	# provide rpm rpmlintrc
	cp package/python-kiwi_crossprepare_plugin-rpmlintrc dist
